...
export default function CommunicationDetailsPage() {
  ...
  async function loadData() { const requestId = ++loadRequestRef.current; setIsLoading(true); setError(''); try { const communicationData = await getCommunication(communicationId); if (requestId !== loadRequestRef.current) return; setCommunication(communicationData); try { setInsight(await getCommunicationInsight(communicationId)) } catch (insightError) { if (insightError.response?.status !== 404) throw insightError } } catch (requestError) { if (requestId === loadRequestRef.current) setError(getApiErrorMessage(requestError)) } finally { if (requestId === loadRequestRef.current) setIsLoading(false) } }
